---
name: agent-browser
description: Use when browser work should run through the upstream agent-browser CLI in this local environment, especially for navigation, snapshots, form interaction, screenshots, authentication reuse, data extraction, or headed browser handoff.
version: "0.1.0"
author: alyosha
dependencies: []
---
...
For OAuth, 2FA, cookie-based auth, and token refresh patterns that are not covered inline here, confirm against the upstream project documentation before relying on them.
...
This registry copy keeps the main skill text only. It does not vendor the upstream companion docs that are referenced in the source repository.

When you need deeper command detail beyond this file:

- Check `verification.md` first for the locally re-verified command and flag subset.
- Run `agent-browser --help` or `agent-browser <command> --help` against the locally installed CLI.
- Fall back to the upstream project documentation for topics not captured in this registry copy.
...
This registry copy does not vendor the upstream shell templates. Recreate a workflow from the inline examples in this file, and verify any command shape you reuse with local `agent-browser --help` output.
