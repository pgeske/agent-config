...
3. **research (for general requests)**
   ...
   - apply recency policy:
     - reddit/forum sources: prefer the last 12 months (allow up to 24 months only for niche products)
     - non-reddit review sources: prefer the last 18 months
     - amazon signal: require meaningful review activity in the last 90 days
   - require at least one recent supporting source (from the last 6–12 months) before making a high-confidence recommendation.

4. **choose candidate + fallback (deal-aware)**
   - pick the best Amazon option that matches user constraints.
   - explicitly compare **quality/performance vs current effective price** (after coupons, clipped deals, lightning deals, promo discounts).
   - if two options are similarly good, prefer the materially cheaper one.
   - if a better product is discounted enough to beat or nearly match a weaker option on price, prefer the better product.
   - do not rely on stale pricing assumptions; re-check price right before final choice.
   - keep one fallback Amazon option in case the first listing is unavailable.

5. **amazon action**
   - use browser automation in the default profile.
...
7. **report back**
   ...
   - include:
     ...
     - price snapshot (including any deal/coupon context that affected the decision)
     - final action:
