...
4. Release desktop work with `computer_use_stop` when finished. Stop, cancellation, errors, and backend idle cleanup invalidate inspected state, **not connection consent**. Never automatically retry an uncertain action. Use `computer_use_revoke` only when the user withdraws consent or asks to disable computer use; never use revoke for routine end-of-task cleanup.
