...
class TestGetOpenaiAuth(unittest.TestCase):
    ...
    def test_api_key_takes_priority(self):
        """OPENAI_API_KEY in last30days config should override Codex auth."""
        file_env = {"OPENAI_API_KEY": "sk-test123"}
        codex_token = _make_jwt({
            "exp": int(time.time()) + 3600,
            "https://api.openai.com/auth": {
                "chatgpt_account_id": "acct_abc123"
            }
        })
        with patch.object(env, 'get_codex_access_token', return_value=(codex_token, "ok")):
    ...
    def test_codex_takes_priority_over_ambient_env_var(self):
        """Ambient OPENAI_API_KEY should not override valid Codex auth."""
        file_env = {}
        codex_token = _make_jwt({
            "exp": int(time.time()) + 3600,
            "https://api.openai.com/auth": {
        ...
        with patch.object(env, 'get_codex_access_token', return_value=(codex_token, "ok")):
            ...
            self.assertEqual(auth.status, "ok")
            self.assertEqual(auth.token, codex_token)
            self.assertEqual(auth.account_id, "acct_abc123")

    @patch.dict(os.environ, {"OPENAI_API_KEY": "sk-from-env"}, clear=False)
    def test_env_var_used_when_codex_missing(self):
        """Ambient OPENAI_API_KEY is still used when Codex auth is unavailable."""
        file_env = {}
