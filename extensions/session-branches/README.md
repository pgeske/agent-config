...
```text
/branch [--with-context] [--new-window] [--name "topic"] [--cwd "path"] [--prompt "task"]
```
...
- `branch` accepts one or more `{ name, prompt, cwd?, withContext?, newWindow? }` tasks and launches them sequentially. A single call can safely create several sibling branches without racing tmux layout updates.
