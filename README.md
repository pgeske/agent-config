...
The managed `~/.config/mcp/mcp.json` enables our [Mac Computer Use MCP](https://github.com/pgeske/mac-computer-use-mcp) through the MCP adapter and keeps Peekaboo disabled. Its compiled release is pinned in `package.json` and installed by bootstrap. One approval enables computer use across apps for the MCP connection. The lightweight server uses `lazy-keep-alive`; routine stop, native-backend idle cleanup, and errors preserve consent between tasks. Explicit `computer_use_revoke` or disconnect/reload clears consent. No apps are permanently pretrusted. Consent covers both recognized native app-access forms, including Chrome's risk-warning variant, without permanent approval. Other native prompts and macOS permissions remain separate, and the agent must still confirm consequential actions. Run `/reload` in existing Pi sessions after syncing. Use `/mcp disable mac-computer-use` followed by `/reload` to disable desktop tools. Project `.mcp.json` files and Pi-specific overrides in `~/.pi/agent/mcp.json` remain machine-local; run `/mcp setup` for interactive additions. The adapter also uses the system credential store. `.env.example` shows environment-variable placeholders for optional personal integrations. The voice bridge is inert until a compatible local broker is listening at `~/.pi/voice/control.sock`.
